...
export function setupWebsiteRoutes(app: express.Express) {
  // Special route for real-withdrawal.html
  app.get('/real-withdrawal.html', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/real-withdrawal.html'));
  });
  
  // Special route for real-transaction-demo.html
  app.get('/real-transaction-demo.html', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/real-transaction-demo.html'));
  });
  
  // Special route for the Tera family painting
  app.get('/tera-family-painting.png', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/tera-family-painting.png'));
  });
  
  // Special route for fallback family painting
  app.get('/tera-mother-children.png', (req, res) => {
    res.sendFile(path.join(process.cwd(), 'public/tera-mother-children.png'));
  });
  
  // Special route for music files with proper MIME type
  ...
  // Special route for welcome images
  app.get('/images/welcome/:filename', (req, res) => {
    const filename = req.params.filename;
    const filePath = path.join(process.cwd(), 'public/images/welcome', filename);
    res.set('Content-Type', 'image/jpeg');
    res.sendFile(filePath);
    console.log('Serving welcome image:', filePath);
  });
