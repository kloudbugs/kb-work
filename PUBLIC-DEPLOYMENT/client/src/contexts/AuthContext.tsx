...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Determine which animation variant to use for admin users
  const getAnimationVariant = () => {
    if (pendingLoginData?.username === 'admin' || pendingLoginData?.role === 'ADMIN') {
      return 'blast'; // Use blast animation for admin users
    } else {
      return 'warp'; // Default animation for regular users
    }
  };
